      ...
      <div class="row justify-content-center">
        ...
        <div class="col-md-12">
          <b-dropdown id="cities" v-bind:text="display" class="m-md-2">
            <b-dropdown-item-button
              ...
              v-on:click="selected = city"
            >{{ city.name }}</b-dropdown-item-button>
          </b-dropdown>
          <b-table
            id="destinations"
            :items="destinationsProvider"
            :fields="fields"
            @row-clicked="hotelSelected"
            striped
            hover
          ></b-table>
        </div>
        <div class="col-md-12">
          <GmapMap
            ...
            :zoom="16"
            :center="{lat: 43.542619, lng: 6.955665}"
          >
            <GmapMarker
              v-for="(marker, index) in poi"
              :key="index"
...
export default {
  ...
  methods: {
    ...
    hotelSelected(record, index) {
      ...
      server.post("/records/select/geo", record.geo).catch(error => {
        console.log(error);
      });
    }
  },
